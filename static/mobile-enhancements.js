...
(function() {
    ...
    function addPlusMinusButtons(input) {
        ...
        minusBtn.className = 'btn-number-minus';
        ...
        plusBtn.type = 'button';
        plusBtn.className = 'btn-number-plus';
        plusBtn.textContent = '+';
        ...
        minusBtn.addEventListener('mouseleave', function() {
            this.style.background = 'var(--light-bg, #f8fafc)';
            this.style.color = 'var(--text-primary, #111827)';
    ...
    function enhanceForms() {
        // Добавляем кнопки к существующим input
        const inputs = document.querySelectorAll('input[type="number"]:not(.number-input-wrapper input)');
        inputs.forEach(input => {
            addPlusMinusButtons(input);
        });
        
        // Используем MutationObserver для отслеживания новых input полей
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType === 1) { // Element node
                        // Проверяем сам узел
                        if (node.tagName === 'INPUT' && node.type === 'number' && !node.parentElement.classList.contains('number-input-wrapper')) {
                            addPlusMinusButtons(node);
                        }
        ...
        // Наблюдаем за изменениями в контейнере формы
        const formContainer = document.querySelector('.workout-form-container') || document.body;
